...
# Forked action
  ...
  if [[ ! $METRICS_USE_PREBUILT_IMAGE =~ ^([Ff]alse|[Oo]ff|[Nn]o|0)$ ]]; then
    ...
    METRICS_IMAGE=ghcr.io/$METRICS_SOURCE/metrics:$METRICS_TAG
    echo "Using pre-built forked version $METRICS_TAG, will pull docker image from GitHub registry (ghcr.io/$METRICS_SOURCE/metrics)"
    if ! docker image pull $METRICS_IMAGE; then
      echo "Failed to fetch docker image from GitHub registry, will rebuild it locally"
