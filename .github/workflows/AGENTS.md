...
## Purpose
The ten GitHub Actions workflows that build, test, format, publish and maintain the project. `ci.yml`
is the spine: it runs on every push to `master` and chains fourteen jobs from testing through docker
...
**The `ci.yml` pipeline, in dependency order:**
...
- Node 20 via `actions/setup-node@v3` and `npm ci` is the standard setup; `actions/checkout@v3` with
  `fetch-depth: 0` wherever git history or another branch is needed.

### Testing Requirements
- `npm run test-contrib` locally mirrors the `lint` job's first step; `npm run linter` mirrors the
  second; `npm run test-metrics` mirrors what the `build` job runs inside docker.
...
### External
- Actions: `actions/checkout@v3`, `actions/setup-node@v3`, `actions/labeler@v4`, `actions/stale@v6`,
...
- Services: `ghcr.io`, `metrics.lecoq.io`, `beta-metrics.lecoq.io`.
