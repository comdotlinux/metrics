...
| Engine pipeline, `metadata.mjs`, `setup.mjs`, `utils.mjs` helpers | `source/app/metrics/AGENTS.md` |
...
Node 20 is pinned in `.tool-versions` (mise and asdf both honor it) and matches the `Dockerfile`. Run `npm ci` with Node 20 first on `PATH`: the native optional packages compile against whatever `node` the install scripts find.

```bash
npm ci                                   # PUPPETEER_SKIP_DOWNLOAD=true if a local Chrome exists
...
```bash
export PUPPETEER_BROWSER_PATH=/usr/bin/google-chrome-stable   # or let postinstall download Chromium
npm run test-metrics                                          # full matrix, ~728 cases, very slow
npx jest --runInBand metrics.test.js -t "GitHub Action Template : classic .*Recent activity"   # one plugin's cases
```

Test names are `<mode> Template : <template> <plugin display name> - <example name>`; modes are `GitHub Action`, `Web instance`, `Web instance (placeholder)`; templates `classic`, `terminal`, `repository`. Example names come from `tests/cases/*.yml`. `npm run test-contrib` only makes sense on a PR branch (it diffs against `origin/master`).
...
4. renders `image.svg` with EJS, including each partial listed in `partials/_.json` whose plugin has data,
5. optimizes CSS/XML, then loads the SVG in puppeteer to measure height with a hidden marker (and screenshot for PNG/JPEG), or emits JSON/Markdown/PDF.
